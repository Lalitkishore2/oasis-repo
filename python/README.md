...
python/
│
├── main.py              # Entry-point bootstrapper (DPI scaling & Tkinter event loop)
├── gui.py               # Custom view panels, theme bindings, and canvas graphs
├── database.py          # CSV engine with exception handling and CRUD logic
├── requirements.txt     # Third-party visualization library dependency
└── bmi_records.csv      # Flat database store
```
